...
from utils.eval_utils import (postprocess_code_lines, extract_identifiers, cal_edit_sim, remove_comments)
...
def process_examples(lang, args):
    ...
    trunc_s = {"task_id": sample["task_id"], "pred": prediction, "target": target, "pred_ids": pred_ids, "target_ids": target_ids}
...
def compute_metric_stmt(output_dir, prompt_file, language="python", ts_lib="utils/build/python-lang-parser.so"):
    ...
    with open(prompt_file, "r") as f_in:
        for l in f_in.readlines():
            ...
            if ex["metadata"]["task_id"] in task_ids:
                examples[ex["metadata"]["task_id"]] = {"prompt": ex["prompt"], "groundtruth": ex["groundtruth"]}
    ...
    exact_match = 0
    with open(f"{output_dir}/prediction_truncated.jsonl", 'w', encoding="utf-8") as pt, open(f"{output_dir}/exact_match_idx.jsonl", 'w') as em:
    ...
    for idx, trunc_s in enumerate(truncated_samples):
        ...
        detailed_results.append({"task_id": trunc_s["task_id"], "em": em_labels[idx], "es": es, "id_em": identifier_em, "id_precision": id_tp / (id_tp + id_fp) if (id_tp + id_fp) != 0 else 0, "id_recall": id_tp / (id_tp + id_fn) if (id_tp + id_fn) != 0 else 0, "id_f1": 2 * id_tp / (2 * id_tp + id_fp + id_fn) if (2 * id_tp + id_fp + id_fn) != 0 else 0})

    em_ratio = round(exact_match / len(samples) * 100, 4)
    edit_sim = round(sum(edit_similarities) / len(edit_similarities), 4)
    id_em_ratio = round(sum(detailed_results[idx]['id_em'] for idx in range(len(detailed_results))) / len(detailed_results) * 100, 4)
    id_precision = round(sum(detailed_results[idx]['id_precision'] for idx in range(len(detailed_results))) / len(detailed_results) * 100, 4)
    id_recall = round(sum(detailed_results[idx]['id_recall'] for idx in range(len(detailed_results))) / len(detailed_results) * 100, 4)
    id_f1 = round(sum(detailed_results[idx]['id_f1'] for idx in range(len(detailed_results))) / len(detailed_results) * 100, 4)
    ...
    with open(f"{output_dir}/results.json", 'w') as f:
        res = {"em": em_ratio, "es": edit_sim, "id_em": id_em_ratio, "id_precision": id_precision, "id_recall": id_recall, "total": len(truncated_samples)}
        f.write(json.dumps(res, indent=2))
    
    return {"em": em_ratio, "es": edit_sim, "id_em": id_em_ratio, "id_f1": id_f1, "total": len(truncated_samples)}
