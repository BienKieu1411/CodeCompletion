...
def compute_ES(target, predictions, passk):
    ...
    for prediction in predictions[:passk]:
        ...
        ES_scores.append(1 - (editdistance.eval(target_str, prediction_str) / max(len(target_str), len(prediction_str))))
...
def eval_repoeval(file_path):
    ...
    return {"em": round(em_ratio * 100, 4), "es": round(edit_sim * 100, 4), "id_em": '-', "id_f1": '-', "total": total}
