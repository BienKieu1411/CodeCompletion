...
def batch_tokenize(tokenizer, texts, max_workers=4):
    with Pool(processes=min(max_workers, len(texts))) as pool:
...
class Retriever(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(args.retriever_model_path, use_fast=False)
        self.args = args
