...
class DeepSeekGenerator:
    ...
    def __init__(self, model_name: str = "deepseek-ai/deepseek-coder-1.3b-base", device: str = "cuda"):
        self.device = device
        print(f"[*] Đang nạp Generator {model_name} lên {device}...")
    ...
    def generate_with_attention(self, prompt: str, retrieved_tokens_len: int, max_new_tokens: int = 32):
        ...
        if attentions and len(attentions) > 0:
            # Lấy attention của token cuối cùng được sinh ra
            last_token_attns = attentions[-1] 
            # Lấy layer cuối cùng (Last Layer) - Shape: (batch=1, num_heads, 1, seq_len)
            last_layer_attn = last_token_attns[-1] 
            # Tính trung bình qua các Heads -> Shape: (1, 1, seq_len)
            cross_attn = last_layer_attn.mean(dim=1) 
        else:
            # Fallback nếu model không hỗ trợ xuất Attention
