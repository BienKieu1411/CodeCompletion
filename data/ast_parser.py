...
class LocalGraph:
    ...
    def to_prompt_string(self) -> str:
        ...
        if self.parent_class:
            res += self.parent_class + "\n\n"
        if self.parent_function:
            res += self.parent_function + "\n\n"
        res += self.local_code
        return res.strip()
...
class ASTQueryExtractor:
    ...
    def _coerce_cursor_idx(cursor_line: int, total_lines: int) -> int:
        ...
        - GraphFRL đang truyền: len(query.split("\\n")) (tức 1-based-ish / end-of-text)
    ...
    def extract_local_graph(self, source_code: str, cursor_line: int, context_lines: int = 15) -> LocalGraph:
        """
    ...
    def _extract_parent_class(self, tree: tree_sitter.Tree, cursor_idx: int, source_code: str) -> Optional[str]:
        ...
        Hiện tại implement chắc chắn cho Python (đủ dùng cho GraphFRL pipeline).
        """
