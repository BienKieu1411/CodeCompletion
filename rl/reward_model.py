...
class CompositeRewardModel:
    ...
    Reward = w1 * R_similarity + w2 * R_struct + w3 * R_judge
    Normalized by EMA baseline for stable PPO updates.
    """
    ...
    def calculate_reward_detailed(
        self, prediction: str, query: str, ground_truth: str,
    ) -> dict:
        """Return individual reward components for logging/analysis."""
        r_sim = compute_edit_similarity(prediction, ground_truth)
        r_struct = compute_structural_similarity(prediction, ground_truth)
        r_judge = self._llm_judge(prediction, query, ground_truth)
        raw = self.w_sim * r_sim + self.w_struct * r_struct + self.w_judge * r_judge
        return {
            "r_similarity": r_sim,
            "r_struct": r_struct,
            "r_judge": r_judge,
