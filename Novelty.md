...
Nếu learned gate không thắng always retrieve, novelty về adaptive retrieval không đứng.
...
- sequential baseline chưa có mode riêng;
...
| sequential baseline | chứng minh co-training hơn train tuần tự |

Code hiện tại đã có hầu hết experiment modes, nhưng sequential baseline vẫn cần thêm nếu muốn claim co-training mạnh.
...
3. Learned gate giảm retrieval noise và không làm mất nhiều case cần retrieval.
4. Adapter mode thắng `retriever_only` hoặc ít nhất cải thiện NLL/output metrics.
5. Co-training thắng sequential baseline khi sequential được bổ sung.
...
Context có thể giảm NLL target nhưng không cải thiện greedy/beam output. Vì vậy paper phải báo cáo cả NLL và generation metrics.
...
Oracle chỉ được dùng để tạo upper-bound preference trong train/analysis. Inference và eval tuyệt đối không được gọi oracle.
...
- `experiment_mode`: `intent_main`, `raw_query_main`, `retriever_only`, `always_retrieve`, `always_skip`, `bm25`, `dense_frozen`.
...
- explicit no-oracle assertion trong eval;
- statistical significance reporting;
- latency/memory reporting.

## 8. Final Novelty Statement

Novelty nên viết như sau:
...
Đây là claim vừa rõ, vừa test được, vừa khớp với code hiện tại.
