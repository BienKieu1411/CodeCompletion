...
class DeterministicQueryProber:
    ...
    So sánh:
        ...
        GraphCoderRL:    Q' = Q + " " + join(identifiers(greedy(Q)))
                     → 1 forward pass, không ảo giác

    Mệnh đề (Claim):
        Cho p_s là xác suất token đúng:
        - AlignCoder: P(query_sạch) = p_s^(n×L)  (giảm theo hàm mũ)
        - GraphCoderRL:   P(query_sạch) = 1 - (1-p_id)  (chỉ phụ thuộc identifier accuracy)
    ...
    def probe(
        ...
        Args:
            ...
            llm: CodeLLMGenerator instance (nếu predicted_text chưa có)
